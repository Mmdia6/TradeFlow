...
def _ledger(db:Session,a:Account,amount:Decimal,kind:TransactionType,user_id:int,order_id:int):
 after=a.available_balance;before=after-amount
 db.add(Transaction(account_id=a.id,user_id=user_id,asset_id=a.asset_id,type=kind,amount=amount,balance_before=before,balance_after=after,reference_type="order",reference_id=str(order_id)))
def _execute(db:Session,o:Order,m:Market,base:Asset,quote:Asset,price:Decimal,user_id:int):
 ...
 if o.side==OrderSide.BUY:
  reserved,actual=o.reserved_amount,qa+fee;qa_account.locked_balance=max(Decimal("0"),qa_account.locked_balance-reserved);leftover=max(Decimal("0"),reserved-actual);qa_account.available_balance+=leftover;ba.available_balance+=qty
...
def create_order(db:Session,user_id:int,payload:dict,key:str)->Order:
 rh=hashlib.sha256(json.dumps(payload,sort_keys=True,default=str).encode()).hexdigest();existing=db.scalar(select(IdempotencyKey).where(IdempotencyKey.user_id==user_id,IdempotencyKey.key==key))
 if existing:
 ...
 if base is None or quote is None:raise ValueError("Market assets are unavailable.")
 ...
 reserve=qty if side==OrderSide.SELL else ep*qty*(Decimal("1")+Decimal(str(settings.trading_fee_rate)));a=_account(db,user_id,base.id if side==OrderSide.SELL else quote.id)
...
def cancel_order(db:Session,user_id:int,order_id:int)->Order:
 ...
 a.locked_balance-=o.reserved_amount;a.available_balance+=o.reserved_amount;_ledger(db,a,o.reserved_amount,TransactionType.ORDER_RELEASE,user_id,o.id);o.reserved_amount=Decimal("0");o.status=OrderStatus.CANCELLED;o.cancelled_at=datetime.utcnow();db.commit();db.refresh(o);return o